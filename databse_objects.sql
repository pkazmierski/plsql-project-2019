...
CREATE OR REPLACE PROCEDURE add_guest (
    ...
    %TYPE, p_status_id guest.status_id%TYPE
) AS
BEGIN
    INSERT INTO guest (
        first_name, last_name, phone, email, nationality, document_id, birth_date, document_type_id, status_id
    ) VALUES (
        p_first_name, p_last_name, p_phone, p_email, p_nationality, p_document_id, p_birth_date, p_document_type_id, p_status_id
    );
...
BEGIN
    INSERT INTO payment (
        amount, payment_type_id
    ) VALUES (
        p_amount, p_payment_type_id
    );
...
BEGIN
    OPEN my_cursor FOR SELECT r.id, rt.name, rt.base_price
