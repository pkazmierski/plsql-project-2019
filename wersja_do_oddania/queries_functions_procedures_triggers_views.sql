...
CREATE OR REPLACE FUNCTION room_filter(p_date_from reservation.checkin_date%TYPE DEFAULT sysdate,
...
BEGIN
    OPEN my_cursor FOR SELECT UNIQUE rm.id
                       FROM room rm
                                LEFT JOIN room_type rt
                                          ON rt.id = rm.room_type_id
                                LEFT JOIN reservation res
                                          ON res.room_id = rm.id
                       WHERE (rm.id NOT IN (SELECT rm2.id
                                            FROM room rm2
                                                     JOIN reservation res2 ON res2.room_id = rm2.id
                                            WHERE ((p_date_from BETWEEN res2.checkin_date AND res2.checkout_date)
                                                OR (p_date_to BETWEEN res2.checkin_date AND res2.checkout_date))))
...
CREATE OR REPLACE FUNCTION room_filter_id(p_date_from reservation.checkin_date%TYPE DEFAULT sysdate,
                                          p_date_to reservation.checkout_date%TYPE DEFAULT sysdate + 7,
                                          p_min_price room_type.base_price%TYPE DEFAULT 0.00,
                                          p_max_price room_type.base_price%TYPE DEFAULT 9999.00,
    ...
    TYPE T_ROOM_RECORD_TYPE IS RECORD (id room.id%TYPE);
...
-- OK cena za pokoj
CREATE OR REPLACE FUNCTION room_price(p_guest_id guest.id%TYPE,
...
CREATE OR REPLACE PROCEDURE generate_year_by_year_report(p_start_year NUMBER, p_end_year NUMBER) AS
...
BEGIN
    ...
    FOR current_year IN p_start_year..p_end_year
        LOOP
            ...
            FROM reservation
            WHERE EXTRACT(YEAR FROM checkin_date) = current_year
...
CREATE OR REPLACE PROCEDURE verify_reservation_status_changed(p_reservation_id reservation.id%TYPE,
...
CREATE
    OR
    REPLACE PROCEDURE add_guest(p_first_name guest.first_name%TYPE,
...
BEGIN
    INSERT INTO guest (first_name, last_name, phone, email, nationality, document_id, birth_date, document_type_id,
                       status_id)
...
CREATE
    OR
    REPLACE PROCEDURE add_payment(p_amount payment.amount%TYPE,
...
BEGIN
    IF UPDATING ('checkin_date') OR INSERTING THEN
        BEGIN
            SELECT to_number(substr(job_name, 11))
            INTO v_checkin_job_id
            FROM user_scheduler_jobs
            WHERE job_action = 'HOTEL.verify_reservation_status_checkin_job(' || to_char(:new.id) || ');';
        EXCEPTION
            WHEN no_data_found THEN
                v_checkin_job_id := 0;
        END;
    ...
    IF UPDATING ('checkout_date') OR INSERTING THEN
        BEGIN
            ...
            FROM user_scheduler_jobs
            WHERE job_action = 'HOTEL.verify_reservation_status_checkout_job(' || to_char(:new.id) || ');';
        EXCEPTION
            WHEN no_data_found THEN
                v_checkout_job_id := 0;
        END;
...
WHERE sysdate NOT BETWEEN res.checkin_date AND res.checkout_date;


-- OK kto w danym dniu nie ma zapłaty za hotel
CREATE OR REPLACE VIEW not_paid_today AS
WITH payment_count AS (SELECT reservation_id,
                              COUNT(*) AS "installments paid"
                       FROM payment
                       GROUP BY reservation_id)
SELECT res.id              AS "reservation id",
       g.first_name,
       g.last_name,
